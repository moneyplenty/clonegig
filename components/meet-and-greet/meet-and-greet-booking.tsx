...
import { Clock, Users, Crown, Star, Zap } from "lucide-react"
...
export function MeetAndGreetBooking({ onBookingComplete }: BookingProps) {
  ...
  const getTierBadge = (tier: string) => {
    switch (tier) {
      ...
      case "avalanche":
        return <Badge className="bg-gold-500/20 text-gold-400 border-gold-500/50">Avalanche</Badge>
      default:
        return null
    }
  }
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
      ...
      {selectedDate && selectedTimeSlot && (
        <Card className="lg:col-span-2 border-electric-700/30 bg-background/50 backdrop-blur-lg">
          <CardHeader>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
              <div className="text-center p-4 rounded-lg bg-electric-500/10 border border-electric-500/30">
                <Calendar className="h-8 w-8 mx-auto mb-2 text-electric-400" />
