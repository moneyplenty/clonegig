...
export function EventsFilters() {
  ...
  const handleClearFilters = () => {
    setDate(undefined)
    setPriceRange([0, 100])
    setEventType({ concerts: false, meetGreets: false, online: false, workshops: false })
    setAccessLevel("all")
    console.log("Cleared Filters")
  }

  return (
    <Card className="w-full md:w-1/4 shrink-0 bg-card/50 backdrop-blur-sm border-kelvin-border">
      <CardHeader>
        ...
        <div>
          ...
          <div className="space-y-2">
            <div className="flex items-center space-x-2">
              <Checkbox
                id="concerts"
                checked={eventType.concerts}
                onCheckedChange={(checked) => handleEventTypeChange("concerts", !!checked)}
            ...
            <div className="flex items-center space-x-2">
              <Checkbox
                id="meetGreets"
                checked={eventType.meetGreets}
                onCheckedChange={(checked) => handleEventTypeChange("meetGreets", !!checked)}
                className="border-electric-700 data-[state=checked]:bg-electric-500 data-[state=checked]:text-electric-100"
              />
              <Label
                htmlFor="meetGreets"
            ...
            <div className="flex items-center space-x-2">
              <Checkbox
                id="workshops"
                checked={eventType.workshops}
                onCheckedChange={(checked) => handleEventTypeChange("workshops", !!checked)}
                className="border-electric-700 data-[state=checked]:bg-electric-500 data-[state=checked]:text-electric-100"
              />
              <Label
                htmlFor="workshops"
